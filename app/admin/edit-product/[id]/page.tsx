...
export default function EditProductPage({ params }: PageProps) {
  ...
  const { productsList, updateProduct, categoriesList } = useAdmin();
  ...
  // Load values once product is found
  useEffect(() => {
    if (product) {
      ...
      setProdOrganic(product.organic || false);
    }
  }, [product]);

  if (!product) {
    return (
      <div className="bg-white border border-neutral-100 rounded-3xl p-8 shadow-sm flex flex-col gap-4 text-center">
        <h2 className="font-serif text-xl font-bold text-neutral-800">Product Not Found</h2>
        <p className="text-sm text-neutral-400">The product you are trying to edit does not exist or has been deleted.</p>
        <button
          onClick={() => router.push('/admin/products')}
          className="mt-2 self-center bg-[#0F2C1F] text-white px-5 py-2.5 rounded-xl font-bold text-xs"
        >
